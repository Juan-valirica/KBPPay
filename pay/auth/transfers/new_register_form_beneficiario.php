...
// Seguridad extra: evitar duplicados exactos
$stmtCheck = $pdo->prepare("
  ...
  LIMIT 1
");
$stmtCheck->execute([$userId, $idNumber, $bank, $accountNumber]);

if ($existing = $stmtCheck->fetch()) {
    $_SESSION['beneficiary_id'] = (int)$existing['id'];
    header('Location: new_register_form_metodo_pago.php');
    exit;
}
...
if (empty($errors)) {
    ...
    $_SESSION['beneficiary_id'] = (int)$pdo->lastInsertId();
